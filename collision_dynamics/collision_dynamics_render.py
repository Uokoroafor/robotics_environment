...
# Path: collision_dynamics/collision_dynamics.py
import arcade
import random
import math
import pymunk
...
class ShelfBounce(arcade.Window):
    ...
    def on_draw(self):
        ...
        arcade.draw_text(
            f"Ball Drop on Shelf Angle: {self.shelf_list[0].angle:.2f} degrees",
            10,
            10,
            arcade.color.BLACK,
            18,
    ...
    def update(self, delta_time):
        ...
        for i, coin in enumerate(self.coin_list):
            ...
            if (
                ...
                < 0
                < -(
                    coin.body.position.y * delta_time
                    + 0.5 * GRAVITY[1] * delta_time**2
                )
            ):
                # Use linear interpolation to estimate the x position at y=0
                fraction_of_time_step = -coin.body.position.y / (
                    coin.body.velocity.y * delta_time
                )
                x_at_y_equals_zero = (
                    coin.body.position.x
                    + coin.body.velocity.x * delta_time * fraction_of_time_step
                )
                # print(f'Coin hits the ground at x = {x_at_y_equals_zero:.2f}.')
                self.bounce_log.append(
                    f" Answer: Coin hits the ground at x = {x_at_y_equals_zero:.2f}."
                )
                arcade.close_window()
