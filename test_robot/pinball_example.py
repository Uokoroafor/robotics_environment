...
class Pinball(arcade.Window):
    ...
    def update(self, delta_time):
        ...
        if self.ball.center_x < BALL_RADIUS or self.ball.center_x > SCREEN_WIDTH - BALL_RADIUS:
            self.ball.change_x *= -1
        if self.ball.center_y < BALL_RADIUS or self.ball.center_y > SCREEN_HEIGHT - BALL_RADIUS:
            self.ball.change_y *= -1
