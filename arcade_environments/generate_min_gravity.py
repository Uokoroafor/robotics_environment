...
with open('../data/freefall/examples_same_steps_min.txt', 'w') as f:
...
with open('../data/freefall/examples_diff_steps_min.txt', 'w') as f:
    for _, row in df_diff.iterrows():
        f.write(f"t0:{row['t_0']} p0:{row['y_0']} and v0:{row['dy_0']}.\n")
        f.write(f"{row['t_1']}?  Answer: {row['y_1']}\n<SEP>\n")

# Generate text for tests
with open('../data/freefall/examples_tests_min.txt', 'w') as f:
