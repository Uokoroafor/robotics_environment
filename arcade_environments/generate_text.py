...
df_tests = pd.read_csv('../freefall/examples_tests.csv')
...
with open('../freefall/examples_same_steps.txt', 'w') as f:
    for _, row in df_same.iterrows():
        f.write(f"An object is dropped at time {row['t_0']} with initial position {row['x_0']}, {row['y_0']} and initial velocity {row['dx_0']}, {row['dy_0']}.\n")
        f.write(f"Acceleration under Gravity is {GRAVITY[1]}. What is the position of the vertical object at time {row['t_1']}?  Answer: {row['y_1']}\n<SEP>\n")

# Generate text for different steps
with open('../freefall/examples_diff_steps.txt', 'w') as f:
    for _, row in df_diff.iterrows():
        f.write(f"An object is dropped at time {row['t_0']} with initial position {row['x_0']}, {row['y_0']} and initial velocity {row['dx_0']}, {row['dy_0']}.\n")
        f.write(f"Acceleration under Gravity is {GRAVITY[1]}. What is the position of the vertical object at time {row['t_1']}?  Answer: {row['y_1']}\n<SEP>\n")

# Generate text for tests
with open('../freefall/examples_tests.txt', 'w') as f:
